...
const AddShows = () => {
    ...
    const fetchNowPlayingMovies = async () => {
        try {
            ...
            toast.error('Failed to fetch movies. Please check TMDB API configuration.');
            setError('Failed to fetch movies. Please check TMDB API configuration.');
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <>
            ...
            {error ? (
                <div className='text-center py-10'>
                    ...
                    <button onClick={fetchNowPlayingMovies} className='bg-primary text-white px-4 py-2 rounded hover:bg-primary/90'>
                        Retry
                    </button>
                </div>
            ) : nowPlayingMovies.length === 0 ? (
                <div className='text-center py-10'>
                    <p className='text-gray-400 mb-4'>No movies currently playing. Please check TMDB API configuration.</p>
                    <button onClick={fetchNowPlayingMovies} className='bg-primary text-white px-4 py-2 rounded hover:bg-primary/90'>
                        Refresh
                    </button>
                </div>
                        ...
                        <div className='group flex flex-wrap gap-4 mt-4 w-max'>
                            {nowPlayingMovies.map((movie) => (
                                <div key={movie.id} className={`relative max-w-40 cursor-pointer group-hover:not-hover:opacity-40 hover:-translate-y-1 transition duration-300`} onClick={() => setSelectedMovie(movie.id)}>
                                    <div className='relative rounded-lg overflow-hiden'>
                                        <img src={image_base_url + movie.poster_path} alt="" className='w-full object-cover brigthness-90' />
