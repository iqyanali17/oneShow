  ...
  return !isLoading ? (
    <>
      ...
      <div className='w-full h-full mt-6 overflow-x-auto'>
        <table className='w-full h-full border-collapse rounded-md overflow-hidden text-nowrap'>
          <thead>
          ...
          <tbody className='text-sm font-light'>
            {bookings.map((item, index) => (
              <tr
                key={index}
                className='border-primary/10 border-b bg-primary/5 even:bg-primary/10'
              >
                {/* FIX: guard against null user */}
                <td className='py-2 min-w-45 pl-5'>
                  {item.user?.name ?? 'Unknown user'}
                </td>

                {/* Optional extra safety for nested fields */}
                <td className='py-2'>
